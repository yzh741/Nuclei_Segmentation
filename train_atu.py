...
from models.at_unet import AttU_Net
...
def main():
    ...
    dsets = {}
    for x in ['train', 'val']:
        img_dir = '{:s}/{:s}'.format(opt.train['img_dir'], x)  # D:/multiorgan/images/train
        target_dir = '{:s}/{:s}'.format(opt.train['label_dir'], x)  # D:/multiorgan/labels/train
        weight_map_dir = '{:s}/{:s}'.format(opt.train['weight_map_dir'], x)  # D:/multiorgan/weight_maps/train
        ...
        num_channels = [3, 1, 3]  # 这个是干嘛的
        dsets[x] = DataFolder(dir_list, post_fix, num_channels, data_transforms[x])

    train_loader = DataLoader(dsets['train'], batch_size=num_gpu * opt.train['batch_size'], shuffle=True,
                              num_workers=opt.train['workers'])
    val_loader = DataLoader(dsets['val'], batch_size=1 * num_gpu, shuffle=False,
                            num_workers=opt.train['workers'])
...
def train(train_loader, model, optimizer, criterion, epoch):
    ...
    for i, sample in enumerate(train_loader):
        ...
        if torch.max(target) == 255:
            target = target / 255
        if target.dim() == 4:  # 由于使用label_encoding,会将3通道的ternary转成一通道（b:0,i:1,bo）
        ...
        loss_CE = loss_map.mean()  # 为什么不直接用cross_entrophy,因为它这里的loss_ce在求均值之前还需和权重图做乘积
        ...
        pred = np.argmax(log_prob_maps.data.cpu().numpy(), axis=1)  # 变成（B,H,W）
...
def validate(val_loader, model, criterion):
    ...
    for i, sample in enumerate(val_loader):
        ...
        output = utils.split_forward(model, input, size, overlap, opt.model['out_c'])  # 一张图切分传入网络得到的结果再组合起来
