...
def color(img_path, save_path):
    # img_path = 'D:/multiorgan/labels_instance/test_diff/TCGA-NH-A8F7-01A-01-TS1.png'
    h = 1000
    w = 1000
    ...
    img = np.array(img)
    print(np.unique(img))
    img_color = np.zeros((1000, 1000, 3), dtype=np.uint8)
    _n_clusters = len(np.unique(img)) - 1  # discard bg 480
    colors = [plt.cm.Spectral(each) for each in np.linspace(0, 1, _n_clusters)]
    for i in range(_n_clusters):
        img_color[img == (i + 1)] = (np.array(colors[i][:3]) * 255).astype('int')
    img_color_pil = Image.fromarray(img_color)
    img_color_pil.save((os.path.join(save_path, name + '.png')))
