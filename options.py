...
class Options:
    def __init__(self, isTrain):
        ...
        # --- training params --- #
        self.train = dict()
        self.train['data_dir'] = 'D:/multiorgan/patches/data_expend2/{}'.format(self.dataset)  # path to data  D:/multiorgan/patches
        ...
        self.test['gpu'] = [0, ]
        self.test['img_dir'] = 'D:/{:s}/images/test_same'.format(self.dataset)
        self.test['label_dir'] = 'D:/{:s}/labels_instance/test_same'.format(self.dataset)  # 测试的时候用label_instance测量
        self.test['tta'] = True
        self.test['save_flag'] = True
        self.test['patch_size'] = 0 if self.dataset == 'GlaS' else 208
        self.test['overlap'] = 80
        self.test['save_dir'] = 'D:/{:s}/{:s}'.format(self.dataset, self.test['epoch'])
        self.test['model_path'] = 'D:/{:s}/FullNet-varCE_MultiOrgan.pth.tar'.format(self.dataset)  # 加载模型
    ...
    def parse(self):
        ...
        if self.isTrain:
            ...
            self.train['img_dir'] = '{:s}/images'.format(  # D:/multiorgan/images/train
                self.train['data_dir'])  # '{:s}/images'.format(self.train['data_dir'])
            self.train['label_dir'] = '{:s}/labels'.format(self.train['data_dir'])  # 改为训练集地址
            ...
            if self.dataset == 'GlaS':
                self.transform['train'] = {
                    ...
                    'label_encoding': 2,
                    'to_tensor': 1,
                    'normalize': [[0.7390994, 0.5199202, 0.64250827], [0.16972609, 0.1711866, 0.13933843]]
                }
                self.transform['val'] = {
                    'scale': 208,
                    'label_encoding': 2,
                ...
                self.transform['train'] = {
                    ...
                    # 'random_rotation': 90,
                    'random_crop': self.train['input_size'],
                    #'RandomResizedCrop': [self.train['input_size'], self.train['input_size'], 1.0],
                    #'ElasticTransform': 0.7,
                    #'label_encoding': 1,
                    #'to_tensor': 1,
