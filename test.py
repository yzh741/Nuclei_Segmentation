...
# test的transform并未用到label_encoding,所以totensor可能会出问题
def main():
    ...
    if save_flag:
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        strs = img_dir.split('/')
        prob_maps_folder = '{:s}/{:s}_prob_maps'.format(save_dir, strs[-1]) #save_dir/test_prob_maps
    ...
    for img_name in img_names:
        ...
        input = test_transform((img,))[0].unsqueeze(0)#把处理好的image拿出来添加一个batch维度

        print('\tComputing output probability maps...')
        prob_maps = get_probmaps(input, model, opt)
        if tta:
            # 变形后的图片经过模型计算后的结果再形变回来和原来的结果比较
    ...
    if eval_flag:
        #[acc, iou, recall, precision, F1, performance, dice, haus]
