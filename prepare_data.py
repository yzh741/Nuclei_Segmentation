...
def main():
    original_data_dir = 'D:/multiorgan'

    annotation_dir = '{:s}/Annotations'.format(original_data_dir)
    img_dir = '{:s}/images'.format(original_data_dir)  # 存放染色均一化图片
    label_instance_dir = '{:s}/labels_instance'.format(original_data_dir)  # mask路径
    label_dir = '{:s}/labels'.format(original_data_dir)  # 存放三元组图
    weightmap_dir = '{:s}/weight_maps'.format(original_data_dir)  # 存放weightmap
    patch_folder = '{:s}/patches'.format(original_data_dir)  # 存放patch

    create_folder(patch_folder)

    # ------ crreate label_instance
    create_instance_label(annotation_dir, label_instance_dir)
    # ------ color normalization for all images
    anchor_img_path = 'D:/multiorgan/Tissue_image/TCGA-G9-6356-01Z-00-DX1.tif'
    color_norm(anchor_img_path, '{:s}/Tissue_image'.format(original_data_dir), img_dir)
...
def create_instance_label(data_dir, save_dir):
    """ create instance labels from instance labels """
    create_folder(save_dir)

    print("Generating instance labels from xml annotation...")
    xml_list = glob.glob(os.path.join(data_dir, '*.xml'))
    for i_, xml in enumerate(xml_list):
        filename = os.path.basename(xml).split('.')[0]
        ...
        instance_label = np.zeros((1000, 1000), dtype=np.uint16)
        for i, cont in enumerate(contours):
            cv2.fillPoly(instance_label, [cont], color=i + 1)
        instance_label_pil = Image.fromarray(instance_label)
        instance_label_pil.save(os.path.join(save_dir, filename + '.png'))
...
def color_norm(anchor_img_path, data_dir, save_dir):
    ...
    anchor_img = np.array(Image.open(anchor_img_path))
